...
/**
 * It is a thread safe and lock-free vector. This class implement algorithm from:<br>
 * 
 ...
 * 
 * This vector supports dynamic expansion of vector in a lock-free manner. The elements are stored
 * in an array of segments. The fist segment (with an index of '0') can contain 8 elements. The
 * <i>n</i>th segment can contain 8*(2**<i>n</i>) elements.
 * 
 * @author Zhi Gan
 * 
...
public class LockFreeList<E> extends AbstractList<E> {
  ...
  /**
   * The descriptor is used to describe current status of vector. It exists so that we can replace
   * two important information in one CAS operation:
   * <ul>
   * <li>size of vector</li>
   * <li>next operation to execute</li>
   * </ul>
   * 
   * If we store above two information in two separated variable, it's possible that data race will
   * occur. Eg, if we update size at first, a pushback() will cause popback return <code>null</code>
   * value. On the contrary, if we modify elements first in a push_back() operation, it might be
   * overwrite by push_back() from other threads.
   * 
   * @param <E>
   */
  ...
  /**
   * Remove the last element in the vector. We don't shrink size of segments here. So the memory of
   * vector never shrink.
   * 
