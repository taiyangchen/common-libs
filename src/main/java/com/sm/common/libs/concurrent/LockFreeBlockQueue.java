...
/**
 * This is a blocking queue based on a lock-free FIFO queue. NULL objects are not allowed in the
 * queue.
 * 
 * The implementation is according to the paper An Optimistic Approach to Lock-Free FIFO Queues by
 ...
 * 
 */
public class LockFreeBlockQueue<E> extends AbstractQueue<E> implements BlockingQueue<E> {
  private volatile Node<E> head, tail;

  @SuppressWarnings("rawtypes")
  private static final AtomicReferenceFieldUpdater<LockFreeBlockQueue, Node> tailUpdater =
      AtomicReferenceFieldUpdater.newUpdater(LockFreeBlockQueue.class, Node.class, "tail");
  @SuppressWarnings("rawtypes")
  private static final AtomicReferenceFieldUpdater<LockFreeBlockQueue, Node> headUpdater =
      AtomicReferenceFieldUpdater.newUpdater(LockFreeBlockQueue.class, Node.class, "head");
  ...
  public E peek() {
    while (true) {
      ...
      if (header == this.head) {
        /*
         * In our algorithm, a dummy node is a special node with a dummy value. It is created and
         * inserted to the queue when it becomes empty as explained above. Since a dummy node does
         * not contain a real value, it must be skipped when nodes are deleted from the queue. The
         * steps for skipping a dummy node are similar to those of a regular dequeue, except that no
         * value is returned. When a dequeue method identifies that the head points to a dummy node
         * and the tail does not, as in Figure 6 Part B in the paper, it modifies the head using a
         * CAS to point to the node pointed by the prev pointer of this dummy node. Then it can
         * continue to dequeue nodes.
         */
        if (tail == header) {
          return null;
        } else {
          ...
          if (tail != head) { // more than 1 node
            if (null != fstNodePrev) {
              if (casHead(head, fstNodePrev)) {
                ...
                if (sizetmp2 < 0) {
                  System.out.println("size = " + sizetmp1 + ":" + sizetmp2 + " , val = " + val + ", dummy = " + dummy
                      + ", head = " + head + ", tail = " + tail + ", fstNodePrev = " + fstNodePrev);
                }
                notifyPut_();
                return val;
              }
