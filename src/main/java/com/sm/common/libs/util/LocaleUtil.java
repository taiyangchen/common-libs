 ...
 * </p>
 * <ul>
 * <li>系统作用域：由JVM所运行的操作系统环境决定，在JVM生命期内不改变。可通过<code>LocaleUtil.getSystem()</code> 取得。</li>
 * <li>默认作用域：在整个JVM中全局有效，可被改变。可通过<code>LocaleUtil.getDefault()</code> 取得。如未明确指定，则取“系统作用域”的值。</li>
 ...
 * </ul>
 * <p>
 * Util工具箱里的其它工具类，当需要时，将从<code>LocaleUtil.getContext()</code> 中取得当前的locale和charset设置。例如：
 * <code>StringEscapeUtil.escapeURL(value)</code> ，如不指定charset
...
public class LocaleUtil {
  ...
  private static Notifier[] getNotifiers() {
    try {
      ...
      List<Notifier> list = new LinkedList<>();

      for (URL file : files) {
        for (String className : StringUtils.split(IOUtil.readText(file.openStream(), "UTF-8", true), "\r\n ")) {
  ...
  private static class AvailableLocales {
    private final Set<String> AVAILABLE_LANGUAGES = new HashSet<>();
    private final Set<String> AVAILABLE_COUNTRIES = new HashSet<>();
