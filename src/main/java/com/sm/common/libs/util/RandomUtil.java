 ...
 * 当并发需要时，如使用相同的<code>Random</code>对象该类提供的方法会有性能问题。可使用 {@link #next(Class, int, int)}或
 * {@link #next(Random, int, int)} 来提升性能
 * 
 * 
...
public abstract class RandomUtil {
  ...
  private static final Computable<String, Random> STRING_BINDS = ConcurrentCache.createComputable();
  ...
  private static Random getRandom(final String string) {
    return STRING_BINDS.get(string, new Callable<Random>() {
      @Override
      public Random call() throws Exception {
        return new Random();
      }
   ...
   * 获取begin和end之间的整数 [begin,end]
   * 
   * @param string 绑定的String，一个类绑定一个Random，提高性能
   * @param begin 起始值
   * @param end 终止值
   * @return begin和end之间的整数
   */
  public static int next(final String string, int begin, int end) {
    if (end < begin) {
      throw new IllegalArgumentException("end must not smaller than begin");
    }

    Random random = getRandom(string);
   ...
   * @return begin和end之间的整数
   */
  public static long next(final String string, long begin, long end) {
    if (end < begin) {
      throw new IllegalArgumentException("end must not smaller than begin");
    }
  ...
  public static double getRndNumP(double pSngBegin, double pSngEnd, double pSngPB, double pSngPE, double pBytP) {
    ...
    if (Arithmetic.div(sngPLen, sngTLen) == Arithmetic.div(pBytP, 100)) {
