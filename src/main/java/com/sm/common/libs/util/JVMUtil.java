 ...
 * @version create on 2017年9月6日 下午3:33:20
 */
public abstract class JVMUtil {
  ...
  /**
   * 适用于 JDK 1.6 from JDK DOC "java.lang.instrument Interface Instrumentation" ... The system class
   * loader supports adding a JAR file to be searched if it implements a method named
   * appendToClassPathForInstrumentation which takes a single parameter of type java.lang.String.
   * The method is not required to have public access. The name of the JAR file is obtained by
   * invoking the getName() method on the jarfile and this is provided as the parameter to the
  ...
  private static Manifest getManifestFromJar(File classpathItem) {
    try (FileInputStream fis = new FileInputStream(classpathItem); JarFile jarFile = new JarFile(classpathItem)) {
  ...
  public static File[] getClasspath(ClassLoader classLoader) {
    ...
    String bootstrap = JAVA_RUNTIME_INFO.sunBootClassPath;
    if (bootstrap != null) {
  ...
  public static final class JavaRuntimeInfo {
    ...
    private final String sunBootClassPath = getSystemProperty("sun.boot.class.path", false);

    private final String sunArchDataModel = getSystemProperty("sun.arch.data.model", false);

    private final String javaVendorUrl = getSystemProperty("java.vendor.url", false);
