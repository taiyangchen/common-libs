 ...
 * 总是首先尝试从<code>Thread.getContextClassLoader()</code>方法取得 <code>ClassLoader</code> 中并装入类和资源。
 ...
 * <li>类<code>B</code>是Web Application中的一个类，是由servlet引擎的<code>ClassLoader</code> 动态装入的</li>
 ...
 * <p>
 * 如果类<code>A</code>使用
 * <code>getClass().getClassLoader().getResource(&quot;C.properties&quot;)</code> ， 就会失败，因为系统
 * <code>ClassLoader</code>不能找到此资源。
 ...
 * @version create on 2017年1月17日 上午11:38:45
 */
public abstract class ClassLoaderUtil {
   ...
   * 
   * @param className 要装入的类名
   * @param classLoader 从指定的<code>ClassLoader</code>中装入类，如果为<code>null</code>，表示从
  ...
  private static boolean getResources(List<URL> urlSet, String resourceName, ClassLoader classLoader,
    ...
    try {
      ...
      logger.error("getResources error", ignore);
    }
  ...
  public static URL getResource(String resourceName, Class<?> referrer) {
    ...
    return (classLoader == null) ? ClassLoaderUtil.class.getClassLoader().getResource(resourceName)
  ...
  public static InputStream getResourceAsStream(String resourceName, Class<?> referrer) {
    ...
    try {
      ...
      // 打开URL失败。can ignore
      logger.error("getResourceAsStream error", ignore);
    }
